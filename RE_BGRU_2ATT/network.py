...
class GRU:
    def __init__(self, is_training, word_embeddings, settings):
        ...
        word_embedding = tf.get_variable(initializer=word_embeddings, name='word_embedding')    # 词向量初始化：预训练模型
        ...
        # tf.contrib.rnn.MultiRNNCell函数可以把rnn按顺序链接，堆叠rnn就是多个rnn进行堆叠
        cell_forward = tf.contrib.rnn.MultiRNNCell([gru_cell_forward] * settings.num_layers)   # 构建多层循环神经网络
        ...
        # embedding layer
        inputs_forward = tf.concat(axis=2,
                                   values=[tf.nn.embedding_lookup(word_embedding, self.input_word),
        ...
        M = tf.tanh(output_h)
        Mw = tf.matmul(tf.reshape(M, [total_num * num_steps, gru_size]), attention_w)
        a = tf.nn.softmax(tf.reshape(Mw,[total_num, num_steps]))
        attention_r = tf.matmul(tf.reshape(a, [total_num, 1, num_steps]), output_h)
        attention_r = tf.reshape(attention_r, [total_num, gru_size])
