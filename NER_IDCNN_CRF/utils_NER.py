...
def clean(params):
    ...
    remove saved model and training logs
    """
    ...
    if os.path.isdir(params.ckpt_path):
        shutil.rmtree(params.ckpt_path)   # shutil.rmtree() 表示递归删除文件夹下的所有子文件夹和子文件
...
def create_model(session, Model_class, path, load_vec, config, id_to_char, logger, is_train=True):
    ...
    ckpt = tf.train.get_checkpoint_state(path)   # 找出训练时保存的模型
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):  # checkpoint_exists检查是否存在具有指定前缀的V1或V2检查点(弃用)
        logger.info("Reading model parameters from %s" % ckpt.model_checkpoint_path)
        model.saver.restore(session, ckpt.model_checkpoint_path)  # 可以找出所有模型中最新的模型
    else:
        logger.info("Created model with fresh parameters.")
        session.run(tf.global_variables_initializer())
        if config["pre_emb"]:
            emb_weights = session.run(model.char_lookup.read_value())  # read_value：返回这个变量的值，在当前的上下文中读取。返回的是一个含有这个值的Tensor
            emb_weights = load_vec(config["emb_file"], id_to_char, config["char_dim"], emb_weights)   # Load word embedding from pre-trained file
            session.run(model.char_lookup.assign(emb_weights))   # assign为变量指定一个新的值
...
def result_to_json(string, tags):
    ...
    for char, tag in zip(string, tags):
        if tag[0] == "S":
            item["entities"].append({"word": char, "start": idx, "end": idx+1, "type":tag[2:]})  # tag: B-LOC  tag[2:]=LOC
        elif tag[0] == "B":
            entity_name += char
            entity_start = idx
        elif tag[0] == "I":
